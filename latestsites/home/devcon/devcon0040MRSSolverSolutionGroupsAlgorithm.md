...
2. Predications in MRS have [variable types](https://blog.inductorsoftware.com/Perplexity/home/mrscon/devhowto0010MRS) beyond the `x`-type variables we've been using. For the examples we'll see here, these can be safely ignored. We'll handle those in a later section.
...
Each variable in an MRS [must have a quantifier that scopes it](https://blog.inductorsoftware.com/Perplexity/home/mrscon/devhowto0010MRS) (the artificial `scope()` predication performed this function in prior examples), and quantifiers always add a numeric criteria to the variable they scope.  Some, like `udef_q` in our example, add the default criteria `between(1, inf)`. This simply means: "at least one". The `_a_q` quantifier means "a single thing", so it adds `between(1, 1)`. 
...
Finally, some variables (`x3` in our example), are defined to be plural by the MRS, as indicated by `NUM: pl` in the [variable properties](https://blog.inductorsoftware.com/Perplexity/home/mrscon/devhowto0010MRS) of `x3`:
...
> - Generate the list of solutions to the modified tree using the approach described in the [previous section](https://blog.inductorsoftware.com/Perplexity/home/devcon/devcon0020MRSSolverSets)

Using a (unshown) world state, and using the approach described in the [previous section](https://blog.inductorsoftware.com/Perplexity/home/devcon/devcon0020MRSSolverSets), the solutions to the modified tree are (let's say):
```
"students lifted a table"
