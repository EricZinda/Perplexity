...
1. Use the code we wrote in the [previous topic](https://blog.inductorsoftware.com/Perplexity/home/pxint/pxint0070GenerateMRSAndTrees) to convert the phrase to MRS and then generate the scope-resolved MRSs for the MRS, go through them in order. 
2. Solve the scope-resolved MRSs using a modification to the `call()` function from [Conjunctions topic](https://blog.inductorsoftware.com/Perplexity/home/pxint/pxint0050Conjunctions) which is called `solve`. Go through these in order.
3. If an error occurs when solving a scope-resolved MRS, get a string for it by calling `generate_message_with_index` (which gets passed as an argument to `interact_once`). We built this in the [English Domain Names section](https://blog.inductorsoftware.com/Perplexity/home/pxint/pxint0120ErrorsConceptualFailures)
...
Last update: 2024-10-28 by Eric Zinda [[edit](https://github.com/EricZinda/Perplexity/edit/main/docs/pxint/pxint0071WhichParseAndTree.md)]{% endraw %}