...
The default `State` object only has a small amount of code for manipulating *application* state, the rest of its implementation manipulates MRS variables. It is literally just one method: `all_individuals()` and we used it in the [Action Verbs topic](pxHowTo070ActionVerbs). Here it is again:
