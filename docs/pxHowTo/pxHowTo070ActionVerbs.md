...
... and `pron(x)` is the last predication to implement (`pronoun_q` is a quantifier that is implemented by the system). `pron(x)` is true when `x` is bound to an object that represents what the specified pronoun is *referring to*. The "specified pronoun" is determined by looking at the [properties](../mrscon/devhowto0010MRS#variable-properties) for the `x` variable to determine if the pronoun is "you". The properties could be: `PERS: 2` (second person: i.e. "you"), `PERS: 3` (third person: i.e. "him/her"), etc. If `x` is bound to an object that represents what that pronoun is referring to, it is `true`. 
