...
Need another design:
    ...
    The scope of a cardinal criteria is a single RSTR value, it gets reset in between rstrs
    The scope of a quantifier can usually be a single RSTR value, but "the" can't work like that
    ...
    There is a difference between predications "filtering" x and "checking" x. Can't get my head around it
        At the end of the day we are looking for solutions that have assignments to the variables that are true given the predications and the world
            A given predication like file(x) is true for both a set of files and a single file
            large_a operates in two modes:
                as a verb, it will only work on sets of 1
                as an adjective, it filters the set down
            Quantifiers send whatever sets of answers are generated by the rstr through the body. 
                The set of items in a given answer can only be as big as the set of items that go through the body
                What does it mean that the rstr returns multiple items?
                    It might mean that it contains something limiting the sets to (say) "4"
