...
With that in place, we can now start reporting errors from predications. As outlined in the [predication contract](pxint0010PredicationContract), "failure" is when the predication is not true for its arguments. So, let's add code at the end of `large_a_1` to record an error when there is a failure. We'll call the new `report_error()` method and pass it what seems like the right error given the code:
