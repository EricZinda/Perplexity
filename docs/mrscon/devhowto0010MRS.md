...
The DELPH-IN [English Resource Grammar (ERG)](ErgTop.md) converts an English phrase into a data structure called an ["Minimal Recursion Semantics Formalism" (MRS)](https://www.cl.cam.ac.uk/~aac10/papers/mrs.pdf) which is a technical representation of human language. The [ACE processor](http://sweaglesw.org/linguistics/ace/), among other processors, processes the grammar and the phrase to produce the MRS formalism and represent it in one of several formats, such as [Simple MRS](MrsRFC.md). Processors can be used with any of the [other DELPH-IN grammars](NorsourceTop.md) to convert other natural languages into the MRS format. While the examples below use English, the concepts apply across the DELPH-IN grammars.
