...
- "What is not soup" --> There isn't a thing
  ...
  - Problem 3: "what is not soup?" -> "thing"
                      ┌────── thing(x3)                ┌────── _soup_n_1(x9)
    which_q(x3,RSTR,BODY)          ┌─ udef_q(x9,RSTR,BODY)
                      └─ neg(e10,ARG1)                 └─ _be_v_id(e2,x3,x9)
    ...
    - Same problem that happens if you say, "what is vegetarian?" ... you get "veggie" for the same reason
      ...
      - Solutions:
        - First: when a predication is used predicatively with an unbound arg, it should probably not return the root, that way we don't get "what is vegetarian?" -> "veggie"
        - Next: "what is vegetarian" should probably return the most detailed concepts.  Same as "what is not soup".  What is that rule?
        - What if we always started with the most detailed hierarchy? Isn't that most often the answer
        - "what is ..." will always have x be singular, but it seems like it really means "what are all the things that are"
        - 
