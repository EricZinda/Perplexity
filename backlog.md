...
- (done) Have the group be able to say, "I'm only the group for this interpretation"
    - First, mark each solution as "generated by this interpretation"
        - MrsTreeLineageGenerator() should remember the interpretation it used
    - Make vocabulary.predication() only call the right one

Fix be_v_id to be a good example:
    - (done) Break out the cases into separate interpretations
    - (done) set the group handlers to only work for a given interpretation
    -
...
Concept refactor:
    ...
    - Make the following scenarios work:
        ...
        - I want a meat/vegetarian dish --> there are more than one
        - What specials do you have?
Let's step back:
    - The notion of seeing if an arbitrary referring expression entails a concept by comparing all of the instances generated
        by the expression to the ones generated by the concept seems valid.
    - Using concept_name in general, seems like a bad idea, but having a "yields something that isa X" could work if we test all instances or
        if we confirm there is not a `not` in there
...
What can be done with a Concept? See Concepts.md ...
- I want a meat dish --> Host: Sorry, I don't know how to give you that.
    - _meat_n_1(x14),_dish_n_of(x8,i19), compound(e13,x8,x14)
                                                                          ┌── _dish_n_of(x8,i19)
                     ┌────── _meat_n_1(x14)                               │
                     │                                        ┌────── and(0,1)
        udef_q(x14,RSTR,BODY)               ┌────── pron(x3)  │             │
                          ...
                          └─ pronoun_q(x3,RSTR,BODY)          │
                                                 └─ _a_q(x8,RSTR,BODY)
                                                                   └─ _want_v_1(e2,x3,x8)
    - Need a way to join two concepts in conjunction
    - probably because it is modelled as "isAdj"?

- Probably don't need level_index anymore?

- It seems like the concepts are not working right
    - Anything that returns concepts should return a base one that is a referring expression, then it should return alternatives?
- How do we know that a concept is the top level referring expression?
    - Options:
        - Concept could just have a property that has a number in it.  0 means "top"?
            - "I want a vegetarian dish" --> _dish_n_of(x8,i14), _vegetarian_a_1(e13,x8) generates an ESLConcept("veggie") where level = 1
                - So this is not a reliable way to determine if it is a top level referring expression
                - Shouldn't this concept be a Concept("Dish") with an additional criteria? In case you wanted to ensure the user qualified the dish for some reason?
        - Maybe we *always* only return the top level referring expression?

- Issue:
    - "table for 2" generates the concept("table") where `maxcapacity` == 2 which does not yield any other concepts and does not == concept('table')
        - The problem is that the concept itself needs to record 'for 2' so that it can be inspected if someone wants to see that, so it has to be there
        - even though all our tables are for 2, which we know because we allow it to be sat at
        - induction should work here
    - I want a steak for 2 will do the same
        - even though all our steaks are for 2, which we know because we allow it to be ordered as a steak
        - induction should work here
    - There are infinitely many concepts that could be entailed by any given concept, so the answer can't be that a given concept is yielding all other concepts
        - Use induction, something like "duck typing"
            - we would need to *exhaust* the list of instances to be sure?
            - which is probably OK in a micro-world
