...
And leaves how to generate them as an exercise for the reader. In this section, we'll go through one approach to generating combinations of solutions that properly deals with what kinds of answers users expect when they say something and allows for some performance optimizations.
...
How many of these solution groups does the system need in order to respond? 1 of them? all of them? If we want the system to respond in the way a user expects, we need to observe the way a human would respond. Here are some:
...
This implies that, for WH-questions (where, what, who, etc), we need to get one answer to respond and see if there is another to let the questioner know there are more. So, we need 2 answers.
...
So, if we decide that we should only pick one of the solution groups to act on for commands, it should be noted that deleting "2 files" or "a file" when there is more than one solution group feels a bit random. For this example, a human might ask, "Do you mean a *random* 2 files?". Certainly "delete files in a folder", if there is only 1 file, feels wrong, just like "Talk to the students that aced the exam" when there is only one student feels wrong.  Either would probably get a response like "OK, but there is only one" to communicate that it isn't *quite* the question asked or to clarify the intent.
...
So, to summarize, we only need to return 2 solution groups in the algorithm we develop. This will give us some room for performance optimization.

## Maximal, Minimal, Subset Solution Groups
Based on the above analysis, we're going to need to retrieve two solution groups (if they exist) when processing a user phrase and show the user (or do) the first group. The second group is just tested to see if it exists and then say "there are more answers here...".
...
In general, subset solutions can be generated any time a criteria for a variable has a range of values that can be true. When the solution group is at the bottom of the range, for that variable it is a "minimal" solution, at the top is a "maximal" solution, anything in between is a "subset" solution. Two examples:
...
All this is to say: if you're only going to give one answer (which we are), it should be a maximal one. To make the system responsive, though, it should start returning an answer as soon as one meets the constraints (i.e. a "minimal" answer), but then keep returning it as the set gets enlarged. That way, the user can see answers as they come.
...
1. Start with `set_list` (a list of sets), which initially is only a single empty set. 
2. When a new solution is found, for each set in `set_list`:
- Build `new_set` by adding the solution to the selected set 
- If `new_set` meets or could meet the criteria (once more rows are added), add it to `set_list`
- If `new_set` actually *did* meet the criteria, also return it as a solution group

This algorithm works because each set in `set_list` becomes a base for creating new combinations when a new solution comes in. And, when that new alternative is created, it also gets added to `set_list` so it can form new combinations. In this way, we generate all possible combinations.
...
Algorithm version 1 only generates alternative solution groups -- it doesn't say which are simply enlarging a solution group with another solution.
...
If we include the lineage when we return a solution group, the caller can see if it is a simple update of a solution group it already has by comparing their lineages.  If they share a prefix, then the new one is a descendent -- it is just "a little more" of that solution. The caller then must use that new lineage as its base to compare from. This is so it doesn't think all the alternatives generated from its current solution group were also "a little more". It is effectively following one set as it grows.
...
So, we will be able to detect if we have more than one unique solution group by picking the lineage that is "the answer" we'll show the user, and then seeing if we can find another solution group that has a different lineage. If there is one, that is a second answer.
...
Last update: 2023-05-08 by EricZinda [[edit](https://github.com/EricZinda/Perplexity/edit/main/docs/devhowto/devhowtoMRSSolverSolutionCombinations.md)]{% endraw %}