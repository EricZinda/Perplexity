...
Next we'll implement `_create_v_1` exactly like we implemented `_delete_v_1` in the [section on action verbs](https://blog.inductorsoftware.com/Perplexity/home/pxhowto/pxHowTo070ActionVerbs). It will use a new operation, which we'll implement next, called `CreateOperation` to do the creating.  For now, we'll just hard code the name of the file to be "newfile":
